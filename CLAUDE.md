...
`./scripts/build-app.sh --install-hooks` requires `jq`, preserves non-FlowState hooks in `~/.claude/settings.json`, replaces only existing commands ending in `flowstate-hook.sh`, and writes `~/.claude/settings.json.bak`.
